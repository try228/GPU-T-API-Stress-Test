...
/// <summary>
/// Universal Intel oneAPI Level Zero compute stress benchmark engine.
/// </summary>
public sealed unsafe class OneApiStressBenchmark
{
    ...
    /// <summary>
    /// Runs the Intel oneAPI Level Zero compute benchmark on the requested GPU device.
    /// </summary>
    public static void Run(CancellationToken hostCt, int initialDuration = 0, int selectedGpuIndex = 0, GpuDeviceDescriptor? targetGpu = null)
    {
        ...
        if (initRes != OneApiNative.ZE_RESULT_SUCCESS)
        {
            throw new InvalidOperationException(
                $"Failed to initialize Level Zero loader (error code {initRes}). " +
                $"Ensure 'libze_loader.so.1' and intel-compute-runtime are installed.");
        }
        ...
        if (driverCount == 0)
        {
            throw new InvalidOperationException("No Level Zero drivers found on this system.");
        }
        ...
        {
            throw new InvalidOperationException("No Level Zero compute-capable devices detected.");
        }

        // Match requested GPU strictly
        int targetIdx = MatchLevelZeroDevice(deviceList, targetGpu, selectedGpuIndex);
        if (targetIdx < 0)
        {
            string available = string.Join(", ", deviceList.Select(d => $"[{d.Index}] {d.Name}"));
            throw new InvalidOperationException(
                $"The selected GPU '[{selectedGpuIndex}] {targetGpu?.Name ?? "Unknown"}' is not supported by Intel oneAPI Level Zero.\n" +
                $"  Available Level Zero device(s): {available}");
        }

        var target = deviceList[targetIdx];
        nint driver = target.Driver;
        nint device = target.Device;
        string devName = target.Name;

        Console.ForegroundColor = ConsoleColor.Green;
        Console.WriteLine($"\n[OneAPIEngine] API Target:      Intel oneAPI Level Zero");
        Console.WriteLine($"[OneAPIEngine] Compute Device:  [{targetIdx}] {devName} (Clock: {target.Clock} MHz)\n");
        Console.ResetColor();

        // 2. Create Context, Queue, and CommandList on target GPU
        ...
        if (ctxRes != 0) throw new InvalidOperationException($"zeContextCreate failed with code: {ctxRes}");
        s_context = ctx;
        ...
        if (qRes != 0) throw new InvalidOperationException($"zeCommandQueueCreate failed with code: {qRes}");
        s_queue = queue;
        ...
        if (cmdRes != 0) throw new InvalidOperationException($"zeCommandListCreate failed with code: {cmdRes}");
        s_cmdList = cmdList;

        // 3. Allocate device VRAM on target GPU
        ...
        if (memRes != 0) throw new InvalidOperationException($"zeMemAllocDevice failed with code: {memRes}");
        s_dBuffer = dptr;

        // 4. Load SPIR-V module
        Console.WriteLine("[OneAPIEngine] Step 4/6: Compiling OpenCL.std SPIR-V in IGC...");
        ...
        fixed (byte* pKName = "stress_kernel\0"u8)
        {
            ...
            if (kernRes != 0) throw new InvalidOperationException($"zeKernelCreate failed with code: {kernRes}");
            s_kernel = kernel;

            nint bufArg = s_dBuffer;
            int argRes = OneApiNative.zeKernelSetArgumentValue(s_kernel, 0, (nuint)sizeof(nint), &bufArg);
            if (argRes != 0) throw new InvalidOperationException($"zeKernelSetArgumentValue failed with code: {argRes}");

            int groupRes = OneApiNative.zeKernelSetGroupSize(s_kernel, 64, 1, 1);
            if (groupRes != 0) throw new InvalidOperationException($"zeKernelSetGroupSize failed with code: {groupRes}");
        }
        ...
        if (appendRes != 0) throw new InvalidOperationException($"zeCommandListAppendLaunchKernel failed with code: {appendRes}");
...
void main() { FragColor = texture(uUiTexture, TexCoord); }";
        ...
        window.Closing += () =>
        {
            ...
            Console.WriteLine("[OneAPIEngine] Intel Level Zero context destroyed cleanly. Exit 0.");
        };

        window.Run();
    }

    private static int MatchLevelZeroDevice(List<(nint Driver, nint Device, string Name, uint Clock, int Index)> devices, GpuDeviceDescriptor? targetGpu, int selectedIndex)
    {
        ...
        if (targetGpu != null)
        {
            ...
            for (int i = 0; i < devices.Count; i++)
            {
                ...
                if ((v == "intel" && (d.Contains("intel") || d.Contains("arc") || d.Contains("graphics") || d.Contains("xe"))) ||
                    (v == "amd" && (d.Contains("radeon") || d.Contains("amd") || d.Contains("gfx"))) ||
        ...
        if (selectedIndex >= 0 && selectedIndex < devices.Count)
            return devices[selectedIndex].Index;
    ...
    {
        0x07230203, 0x00010000, 0x00080001, 0x0000000a, 0x00000000,
        0x00020011, 0x00000004,
        0x00020011, 0x00000006,
        0x00020011, 0x0000000b,
        0x0005000b, 0x00000001, 0x6e65704f, 0x732e4c43, 0x00006474,
        0x0003000e, 0x00000002, 0x00000002,
        0x0007000f, 0x00000006, 0x00000005, 0x65727473, 0x6b5f7373, 0x656e7265, 0x0000006c,
        0x00030003, 0x00000003, 0x00030d40,
        0x00020013, 0x00000002,
        0x00040015, 0x00000003, 0x00000020, 0x00000000,
        0x00040020, 0x00000004, 0x00000005, 0x00000003,
        0x00040021, 0x00000006, 0x00000002, 0x00000004,
        0x0004002b, 0x00000003, 0x00000007, 0x00000001,
        0x00050036, 0x00000002, 0x00000005, 0x00000000, 0x00000006,
        0x00030037, 0x00000004, 0x00000008,
        0x000200f8, 0x00000009,
        0x0003003e, 0x00000008, 0x00000007,
        0x000100fd,
        0x00010038
    };
