...
| `vkd3d`                      | Direct3D 12 over Vulkan via Wine upstream V# GPU-T Stress Test & Benchmark Sidecar
...
Compute APIs such as **Vulkan, OpenCL, CUDA, ROCm, and oneAPI**, as well as translation layers such as **DXVK** and **VKD3D-Proton**/**VKD3D**, directly target the GPU selected with:

```bash
-g <index|name>
```

### OpenGL Multi-GPU Behavior

Native OpenGL backends (`gl`, `gles`) and render through the display server's default screen context.

To stress a secondary GPU using an OpenGL-compatible backend, use Zink:
...
The CUDA backend dynamically detects whether it is running with native NVIDIA drivers or through **ZLUDA** translation on supported AMD hardware.
